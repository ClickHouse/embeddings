...
./clickhouse-new client --host "$H" --secure --user default --password "$PW" --query "TRUNCATE TABLE mmcommons.yfcc_metadata"
sqlite3 -separator $'\t' mmcommons/metadata/yfcc100m_dataset.sql "SELECT $COLS FROM yfcc100m_dataset LIMIT 500000" \
...
echo "INSERT_RC=$?"
./clickhouse-new client --host "$H" --secure --user default --password "$PW" --query "SELECT count() AS rows FROM mmcommons.yfcc_metadata"
