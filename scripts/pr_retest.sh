...
echo "########## SEARCH with parallel replicas forced per-query (bits=16) ##########"
CC --query_id pr_now --time --query "
  SELECT id, by,
...
CC --query "
  ...
  WHERE query_id='pr_now' AND type='QueryFinish'
