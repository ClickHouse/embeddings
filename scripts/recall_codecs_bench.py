...
Grid: 7 models x 2 codecs x 3 k x 3 index-fetch multipliers = 126 configs, each averaged over the pool's
query points (100 by default). k and the multiplier both vary because the Explorer exposes both, and the
headline metric is recall@k -- the fraction of the true top-k the search returned, which is exactly what
the report panel puts on screen.

k MUST vary. Recall depends on the CANDIDATE POOL, k x multiplier, not on either alone: measured on
hn_nomic, k=10/x10 and k=100/x1 both fetch 100 candidates and both score recall@10 = 0.956, while a real
k=10/x1 search fetches 10 candidates and scores 0.512. An earlier version of this harness pinned k=100
and reported "recall@10", which made every multiplier look benign -- it was scoring the top 10 of a
100-candidate rescore, not a k=10 search.
...
KS    = (10, 100, 1000)     # top-k per search; matches the Explorer's `nearest` chips
MULTS = (1, 5, 10)          # vector_search_index_fetch_multiplier; matches the `multiplier` chips
...
GT = {}     # label -> [(q_md5, {k: set(true top-k)}), ...] ordered by q_idx
POOL_ROWS = {}
for corpus, ds, label, dim in MODELS:
    body, _, _ = post(
        f"SELECT q.q_idx AS q_idx, q.q_md5 AS q_md5, g.gt10 AS gt10, g.gt100 AS gt100, g.gt1000 AS gt1000 "
    ...
    GT[label] = [(r['q_md5'], {10: set(r['gt10']), 100: set(r['gt100']), 1000: set(r['gt1000'])})
                 for r in rows]
...
# ---- one search --------------------------------------------------------------------------------
def search(label, codec, k, mult, q_md5, codes='1'):
...
def run(cfg):
    corpus, ds, label, dim, codec, k, mult = cfg
    rk = r10 = 0.0
    bpr_sum = rows_sum = 0.0
    for q_md5, gts in GT[label]:
        got, rr, rb = search(label, codec, k, mult, q_md5)
        bpr_sum += rb / max(rr, 1); rows_sum += rr
        rk  += len(gts[k] & set(got)) / float(k)        # recall@k: did we get the true top-k?
        r10 += len(gts[10] & set(got[:10])) / 10.0      # recall@10: comparable across every k
    n = len(GT[label])
    ...
    return dict(corpus=corpus, dataset=ds, codec=codec, k=k, multiplier=mult,
                candidates=k * mult, dim=dim, code_bytes=code_bytes,
                bytes_per_row=round(bpr_sum / n, 1), quantized=quantized,
                recall_at_k=round(rk / n, 4), recall10=round(r10 / n, 4))

# ---- control: codes=0 must reproduce the ground truth exactly, else the harness is wrong ------
print('control (codes=0, expect recall@k = 1.0 at every k):', flush=True)
ctrl_bad = 0
for corpus, ds, label, dim in MODELS:
    out = []
    for k in KS:
        ck = 0.0
        for q_md5, gts in GT[label]:
            got, _, _ = search(label, 'rabitq', k, 1, q_md5, codes='0')
            ck += len(gts[k] & set(got)) / float(k)
        ck /= len(GT[label]); out.append((k, ck))
    bad = any(v != 1.0 for _, v in out)
    ctrl_bad += bad
    print(f"  {corpus}/{ds}: " + '  '.join(f"r@{k}={v:.4f}" for k, v in out)
          + ('   *** GROUND TRUTH MISMATCH ***' if bad else ''), flush=True)
if ctrl_bad:
    raise SystemExit(f"aborting: {ctrl_bad} model(s) failed the exactness control")

configs = [(c, ds, label, dim, codec, k, mult)
           for c, ds, label, dim in MODELS for codec in CODECS for k in KS for mult in MULTS]
...
with ThreadPoolExecutor(max_workers=6) as ex:
    for res in ex.map(run, configs):
        ...
        print(f"  [{done}/{len(configs)}] {res['corpus']}/{res['dataset']} {res['codec']} "
              f"k={res['k']} x{res['multiplier']}  r@k={res['recall_at_k']:.3f} r@10={res['recall10']:.3f}"
              f"{'' if res['quantized'] else '   *** FULL-PRECISION FALLBACK ***'}", flush=True)

COLS = ['corpus', 'dataset', 'codec', 'k', 'multiplier', 'candidates', 'dim', 'code_bytes',
        'bytes_per_row', 'quantized', 'recall_at_k', 'recall10']
dest = os.path.join(BASE, 'results/recall_codecs.csv')
with open(dest, 'w', newline='') as f:
    w = csv.DictWriter(f, fieldnames=COLS); w.writeheader()
    for r in sorted(results, key=lambda r: (r['corpus'], r['dataset'], r['codec'], r['k'], r['multiplier'])):
        w.writerow(r)
...
if fell_back:
    ...
    for r in fell_back:
        print(f"  {r['corpus']}/{r['dataset']} {r['codec']} k={r['k']} x{r['multiplier']} ({r['bytes_per_row']}B/row)", flush=True)
