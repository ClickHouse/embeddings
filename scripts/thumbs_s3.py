...
Gamma-correct 75x75 (cv2 INTER_AREA in linear light). stdout: parquet (md5, thumb Array(UInt8) 16875). arg1=NT."""
...
_S2L = np.where(_x <= 0.04045, _x/12.92, ((_x+0.055)/1.055)**2.4).astype(np.float32)
def fetch(m):
...
def thumb(jp):
    ...
    out = cv2.resize(_S2L[a], (SIZE, SIZE), interpolation=cv2.INTER_AREA)          # gamma-correct area downscale
...
out_q = queue.Queue(maxsize=256)                                                   # backpressure -> bounded RAM
def worker():
    while True:
        m = in_q.get()
