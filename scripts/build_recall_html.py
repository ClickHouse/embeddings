...
<div class=sub><a href="index.html">&#8592; Embeddings Explorer</a> &middot; per-dataset random sample (selectable), 20 random queries, exact-cosine ground truth. Green = 1.0, red = 0.0.</div>
<div class=ctl>
...
function render() {
  ...
  for (const b of bits) {
    h += `<tr><th class=cor>${b}</th>` + dims.map(d => {
      const v = m[b + '_' + d];
      return v==null ? '<td></td>' : `<td class="cell${pareto.has(b+'_'+d)?' pareto':''}" data-k="${b}_${d}" data-v="${v}" data-bytes="${b*d/8}" style="background:${color(v)}" title="bits=${b} dims=${d}">${v.toFixed(2)}<span class=sz>${bytesPerVec(b,d)}</span></td>`;
    }).join('') + '</tr>';
  }
  h += '</table>';
...
const _chart = document.getElementById('chart');
const _legval = document.getElementById('legval');
// align the legend + chart left edges with the first VALUE column (past the bits/dims header column)
function alignLeft(){
  const t = document.querySelector('#tbl table'); if (!t || !t.rows[0] || !t.rows[0].cells[1]) return;
  const pad = (t.rows[0].cells[1].getBoundingClientRect().left - t.getBoundingClientRect().left) + 'px';
  document.querySelector('.legend').style.paddingLeft = pad;
  document.querySelector('.chartwrap').style.paddingLeft = pad;
}
...
const _dotsOff  = () => _chart.querySelectorAll('circle.hi').forEach(el => el.classList.remove('hi'));
const _cellsOff = () => _tbl.querySelectorAll('td.cell.xhi').forEach(td => td.classList.remove('xhi'));
const _clear = () => { _tbl.querySelectorAll('td.cell.dim,td.cell.best').forEach(td => td.classList.remove('dim','best')); _dotsOff(); _legval.textContent = ''; };
// hover a cell -> highlight cells with >= its recall, and its matching chart dot(s)
_tbl.addEventListener('mouseover', e => { const c = e.target.closest('td.cell'); if (!c) { _clear(); return; }
  applyThreshold(+c.dataset.v);
  _dotsOff(); _chart.querySelectorAll(`circle[data-k="${c.dataset.k}"]`).forEach(el => el.classList.add('hi')); });
_tbl.addEventListener('mouseleave', _clear);
// hover a chart dot -> highlight its matching table cell
_chart.addEventListener('mouseover', e => { const c = e.target.closest('circle'); _cellsOff();
  const k = c && c.getAttribute('data-k'); if (!k) return;
  const cell = _tbl.querySelector(`td.cell[data-k="${k}"]`); if (cell) cell.classList.add('xhi'); });
_chart.addEventListener('mouseleave', _cellsOff);
